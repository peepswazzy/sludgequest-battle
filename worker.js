...
<div class="grid">
<button data-mode="adaptive">🔥 <b>Adaptive Training 2.0</b><small>Finds your weak areas and attacks them.</small></button>
...
<div class="top"><div><p class="eyebrow">SLUDGEMATCH V11.9</p><h2>📣 Update Board</h2></div><button id="updateHome" class="secondary">Home</button></div>
<p class="status">What stay new in SludgeMatch. Newest updates stay on top.</p>
<div class="update-row"><div class="update-version"><b>V11.9 — Update Board</b><span class="pill">NEW</span></div><ul class="update-list"><li>New in-game Update Board on the home screen.</li><li>Shows recent features and version history so operators know what changed.</li><li>NEW badge clears after you open the board.</li></ul></div>
...
let st=JSON.parse(localStorage.getItem("sludgequest")||"null")||{xp:0,streak:0,badges:[],stats:{}};st.battleWins=st.battleWins||0;st.battleLosses=st.battleLosses||0;st.battleTopics=st.battleTopics||{};st.seasonXp=st.seasonXp||0;st.seasonWins=st.seasonWins||0;st.seasonLosses=st.seasonLosses||0;st.seasonBossWins=st.seasonBossWins||0;st.seasonId=st.seasonId||"S1";st.operatorId=st.operatorId||("OP-"+Math.random().toString(36).slice(2,10)+Date.now().toString(36).slice(-4));st.operatorName=st.operatorName||"Operator";st.operatorPlant=st.operatorPlant||"Unassigned Plant";st.operatorGrade=st.operatorGrade||4;st.questProgress=st.questProgress||{};st.lastSeenUpdate=st.lastSeenUpdate||"";const CURRENT_UPDATE="11.9";st.bestBattleScore=Number(st.bestBattleScore)||0;st.bestBattleType=st.bestBattleType||"";st.battleHighScores=st.battleHighScores||{};topics.forEach(t=>{st.stats[t]||={c:0,n:0};st.battleTopics[t]||={c:0,n:0}});
...
function show(id){["home","updateBoard","questHub","solo","lobby","waiting","battleGame","results","leaderboard"].forEach(x=>$(x).classList.toggle("hidden",x!==id));if(id==="home")refreshUpdateBadge()}
...
let mode,deck=[],i=0,current,answered=false,correct=0,soloTotal=10,activeQuest=null,adaptivePerf={},adaptiveMisses=[],adaptiveUsed=new Set();
...
function adaptivePool(){let all=[...bank];questTrains.forEach(tr=>tr.questions.forEach(q=>all.push({...q,y:q.y||"adaptive"})));let seen=new Set();return all.filter(q=>{let k=q.q;if(seen.has(k))return false;seen.add(k);return true})}
function topicAccuracy(t){let s=st.stats[t]||{c:0,n:0};return s.n?s.c/s.n:.55}
function adaptiveLevel(t){let s=st.stats[t]||{c:0,n:0},acc=topicAccuracy(t);if(s.n<3)return 2;if(acc>=.88)return 5;if(acc>=.75)return 4;if(acc>=.58)return 3;return 2}
...
function adaptiveLabel(){let vals=Object.entries(adaptivePerf).filter(([t,v])=>v.n).sort((a,b)=>(a[1].c/a[1].n)-(b[1].c/b[1].n));if(!vals.length)return "ADAPTIVE 2.0";let weak=vals[0][0];return "ADAPTIVE 2.0 • FOCUS: "+weak.toUpperCase()}
