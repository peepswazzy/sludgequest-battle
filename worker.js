...
function profile(){$("rank").textContent=rank();$("xp").textContent=st.xp;$("battleRecord").textContent=`${st.battleWins}-${st.battleLosses}`;$("stats").innerHTML=topics.map(t=>{let s=st.stats[t];return \`<p>\${t}: \${s.n?Math.round(s.c/s.n*100)+"%":"No data"}</p>\`}).join("");$("badges").innerHTML=st.badges.length?st.badges.map(x=>\`<span class="badge">\${x}</span>\`).join(""):"No achievements yet. 🤙"}
