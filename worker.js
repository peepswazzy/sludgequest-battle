...
<section id="leaderboard" class="card hidden"><div class="top"><div><p class="eyebrow">SLUDGEMATCH V10</p><h2>Leaderboard 🏆</h2></div><button id="leaderHome" class="secondary">Home</button></div><div class="tabs"><button id="opTab" class="active">Operators</button><button id="plantTab">🏭 Plant Championship</button></div><p id="leaderStatus" class="status">Loading rankings…</p><div id="leaderList"></div></section>
...
let leaderMode="operators";async function loadLeaderboard(mode){leaderMode=mode;$("leaderStatus").textContent="Loading live rankings…";$("leaderList").innerHTML="";try{let r=await fetch("/api/leaderboard?mode="+encodeURIComponent(mode),{cache:"no-store"});if(!r.ok)throw new Error("bad");let data=await r.json();let rows=data.rows||[];$("leaderStatus").textContent=rows.length?(mode==="plants"?"🏭 Live Plant Championship — every operator helps the plant.":"Live SludgeMatch operator rankings"):"No rankings yet — finish a battle or save your profile.";$("leaderList").innerHTML=rows.map((x,i)=>{let crown=mode==="plants"&&i===0?" 🏆":"";let extra=mode==="plants"&&x.topOperator?"<div class='leader-meta'>Top Operator: "+escapeHtml(x.topOperator)+" • Team XP: "+Number(x.xp||0).toLocaleString()+"</div>":"";return "<div class='leader-row'><div class='leader-rank'>#"+(i+1)+"</div><div><b>"+escapeHtml(x.name)+crown+"</b><div class='leader-meta'>"+escapeHtml(x.meta||"")+"</div>"+extra+"</div><b>"+Number(x.score||0).toLocaleString()+" <span class='leader-meta'>Rating</span></b></div>"}).join("")}catch(e){$("leaderStatus").textContent="Leaderboard unavailable right now. Your local progress is still safe."}}
  ...
  async fetch(request) {
    ...
    if (request.headers.get("Upgrade") !== "websocket") {
      ...
      if (url.pathname === "/internal/leaderboard") {
        ...
        const plants={}; for(const p of eligible){let k=p.plant;plants[k]??={name:k,xp:0,wins:0,losses:0,operators:0,topOperator:"",topScore:-1};let opScore=(Number(p.xp)||0)+(Number(p.wins)||0)*250;plants[k].xp+=Number(p.xp)||0;plants[k].wins+=Number(p.wins)||0;plants[k].losses+=Number(p.losses)||0;plants[k].operators++;if(opScore>plants[k].topScore){plants[k].topScore=opScore;plants[k].topOperator=p.name}}
        const rows=Object.values(plants).map(p=>({name:p.name,meta:p.operators+" operator"+(p.operators===1?"":"s")+" • Battle W-L "+p.wins+"-"+p.losses,xp:p.xp,wins:p.wins,losses:p.losses,operators:p.operators,topOperator:p.topOperator,score:p.xp+p.wins*250})).sort((a,b)=>b.score-a.score).slice(0,25);return Response.json({rows})
      }
