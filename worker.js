...
function profile(){$("rank").textContent=rank();$("xp").textContent=st.xp;$("battleRecord").textContent=String(st.battleWins)+"-"+String(st.battleLosses);$("profileLine").textContent=st.operatorName+" • "+st.operatorPlant+" • Grade "+st.operatorGrade;$("stats").innerHTML=topics.map(t=>{let s=st.stats[t];return \`<p>\${t}: \${s.n?Math.round(s.c/s.n*100)+"%":"No data"}</p>\`}).join("");$("badges").innerHTML=st.badges.length?st.badges.map(x=>\`<span class="badge">\${x}</span>\`).join(""):"No achievements yet. 🤙"}
  ...
  async fetch(request) {
    ...
    if (request.headers.get("Upgrade") !== "websocket") {
      ...
      if (url.pathname === "/internal/leaderboard") {
        ...
        if(mode==="operators"){const rows=profiles.map(p=>({name:p.name,meta:p.plant+" • Grade "+p.grade+" • "+p.wins+"-"+p.losses,score:p.xp+p.wins*250})).sort((a,b)=>b.score-a.score).slice(0,25);return Response.json({rows})}
        const plants={}; for(const p of profiles){let k=p.plant||"Unassigned Plant";plants[k]??={name:k,xp:0,wins:0,losses:0,operators:0};plants[k].xp+=p.xp;plants[k].wins+=p.wins;plants[k].losses+=p.losses;plants[k].operators++}
