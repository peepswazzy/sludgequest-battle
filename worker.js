...
.room{text-align:center;font-size:2.1rem;font-weight:900;background:var(--card2);border:1px dashed var(--accent);border-radius:16px;padding:18px}.alarm,.warning{padding:11px;border-radius:12px;margin:14px 0}.timer{height:9px;background:var(--card2);border:1px solid var(--line);border-radius:999px;overflow:hidden;margin:12px 0}.timer>div{height:100%;width:100%;background:var(--accent);transition:width .1s linear}.round-result{background:var(--card2);border:1px solid var(--line);border-radius:14px;padding:12px;margin:12px 0}.round-result b{color:var(--accent)}.alarm{background:#371b18;border:1px solid #82463e;color:#ffd7d0;font-weight:900}.warning{background:#2b2613;border:1px solid #806f32}.hidden{display:none!important}
...
let ws=null,room="",role="",host="",guest="",bc=10,br=0,bdeck=[],scores={host:0,guest:0},ans={host:false,guest:false},choices={host:null,guest:null},answerMs={host:null,guest:null},roundStarted=0,timerId=null;
...
$("joinBtn").onclick=async()=>{room=$("roomInput").value.trim().toUpperCase();role="guest";guest=$("guestName").value.trim()||"Operator 2";if(!/^WW-[0-9]{6}$/.test(room)){$("battleStatus").textContent="Room code should look like WW-123456.";return}$("battleStatus").textContent="Finding server room…";try{await connectSocket(room,"join",guest);$("roomCode").textContent=room;$("hostDisplay").textContent="Host";$("guestDisplay").textContent=guest;$("waitingText").textContent="Connected. Waiting for host to start…";show("waiting")}catch(e){$("battleStatus").textContent="Room not found or Battle server unavailable."}};
$("startBtn").onclick=()=>{if(role!=="host"||!ws||ws.readyState!==WebSocket.OPEN)return;let p=[];while(p.length<bc)p.push(...shuffle(bank));bdeck=p.slice(0,bc).map(q=>{let order=shuffle(q.a.map((_,i)=>i));return {...q,a:order.map(i=>q.a[i]),c:order.indexOf(q.c),alarm:Math.random()<.2}});scores={host:0,guest:0};send("start",{host,guest,deck:bdeck,scores})};
...
function resolveRound(){let q=bdeck[br],base=q.alarm?150:100;let detail={};["host","guest"].forEach(r=>{let ok=choices[r]===q.c;let speed=ok?Math.max(0,Math.round((20000-(answerMs[r]||20000))/1000)*5):0;let earned=ok?base+speed:0;scores[r]+=earned;detail[r]={ok,earned,speed,ms:answerMs[r]}});send("result",{correct:q.c,scores,detail})}
function reveal(c,detail={}){clearInterval(timerId);[...$("battleA").children].forEach((b,j)=>{b.disabled=true;if(j===c)b.classList.add("correct");if(j===choices[role]&&j!==c&&choices[role]>=0)b.classList.add("wrong")});$("s1").textContent=scores.host;$("s2").textContent=scores.guest;let h=detail.host||{},g=detail.guest||{};$("roundResult").innerHTML=`<b>Round result</b><br>${host}: ${h.ok?"✅":"❌"} +${h.earned||0} &nbsp; • &nbsp; ${guest}: ${g.ok?"✅":"❌"} +${g.earned||0}`;$("roundResult").classList.remove("hidden");$("gameStatus").textContent=`Correct answer: ${bdeck[br].a[c]}`;if(role==="host")setTimeout(()=>{if(br===bdeck.length-1)send("finish",{scores});else send("next",{br:br+1})},2600)}
function finishBattle(){clearInterval(timerId);show("results");const w=scores.host===scores.guest?"Tie game 😂":(scores.host>scores.guest?host:guest)+" wins da shift! 👑";$("winner").textContent=w;$("finalScore").textContent=`${host}: ${scores.host} — ${guest}: ${scores.guest}`}
