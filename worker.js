...
.room{text-align:center;font-size:2.1rem;font-weight:900;background:var(--card2);border:1px dashed var(--accent);border-radius:16px;padding:18px}.alarm,.warning{padding:11px;border-radius:12px;margin:14px 0}.timer{height:9px;background:var(--card2);border:1px solid var(--line);border-radius:999px;overflow:hidden;margin:12px 0}.timer>div{height:100%;width:100%;background:var(--accent);transition:width .1s linear}.round-result{background:var(--card2);border:1px solid var(--line);border-radius:14px;padding:12px;margin:12px 0}.round-result b{color:var(--accent)}.plant-card{background:var(--card2);border:1px solid var(--line);border-radius:14px;padding:12px;margin:10px 0}.record{font-size:1.2rem;font-weight:900}.weak{color:#ffd08a}.boss{border:1px solid #8d6d2d;background:#2c2412;color:#ffe3a0;padding:11px;border-radius:12px;margin:12px 0;font-weight:900}.alarm{background:#371b18;border:1px solid #82463e;color:#ffd7d0;font-weight:900}.warning{background:#2b2613;border:1px solid #806f32}.hidden{display:none!important}
...
<section id="home">
<div class="profile card"><div><span>Rank</span><b id="rank">OIT</b></div><div><span>XP</span><b id="xp">0</b></div><div><span>Battle W-L</span><b id="battleRecord">0-0</b></div></div>
...
let st=JSON.parse(localStorage.getItem("sludgequest")||"null")||{xp:0,streak:0,badges:[],stats:{}};st.battleWins=st.battleWins||0;st.battleLosses=st.battleLosses||0;st.battleTopics=st.battleTopics||{};topics.forEach(t=>{st.stats[t]||={c:0,n:0};st.battleTopics[t]||={c:0,n:0}});
...
function profile(){$("rank").textContent=rank();$("xp").textContent=st.xp;$("battleRecord").textContent=String(st.battleWins)+"-"+String(st.battleLosses);$("stats").innerHTML=topics.map(t=>{let s=st.stats[t];return \`<p>\${t}: \${s.n?Math.round(s.c/s.n*100)+"%":"No data"}</p>\`}).join("");$("badges").innerHTML=st.badges.length?st.badges.map(x=>\`<span class="badge">\${x}</span>\`).join(""):"No achievements yet. 🤙"}
...
$("liveBtn").onclick=()=>{show("lobby");$("battleStatus").textContent="Plant vs. Plant ready — rooms are coordinated online."};$("battleHome").onclick=()=>{closeWs();show("home")};
