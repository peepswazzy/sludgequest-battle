...
<div class="top"><div><p class="eyebrow">SLUDGEMATCH V11.10.1</p><h2>📣 Update Board</h2></div><button id="updateHome" class="secondary">Home</button></div>
<p class="status">What stay new in SludgeMatch. Newest updates stay on top.</p>
<div class="update-row"><div class="update-version"><b>V11.10.1 — Adaptive Grade Select Polish</b><span class="pill">NEW</span></div><ul class="update-list"><li>All Grades now uses the ⚙️ operator/equipment icon.</li><li>Adaptive Training still lets the operator choose Grade 1, 2, 3, 4, or All Grades.</li><li>Weak-area weighting and difficulty adjustment stay active inside the selected grade.</li></ul></div>
...
let st=JSON.parse(localStorage.getItem("sludgequest")||"null")||{xp:0,streak:0,badges:[],stats:{}};st.battleWins=st.battleWins||0;st.battleLosses=st.battleLosses||0;st.battleTopics=st.battleTopics||{};st.seasonXp=st.seasonXp||0;st.seasonWins=st.seasonWins||0;st.seasonLosses=st.seasonLosses||0;st.seasonBossWins=st.seasonBossWins||0;st.seasonId=st.seasonId||"S1";st.operatorId=st.operatorId||("OP-"+Math.random().toString(36).slice(2,10)+Date.now().toString(36).slice(-4));st.operatorName=st.operatorName||"Operator";st.operatorPlant=st.operatorPlant||"Unassigned Plant";st.operatorGrade=st.operatorGrade||4;st.questProgress=st.questProgress||{};st.lastSeenUpdate=st.lastSeenUpdate||"";const CURRENT_UPDATE="11.10.1";st.bestBattleScore=Number(st.bestBattleScore)||0;st.bestBattleType=st.bestBattleType||"";st.battleHighScores=st.battleHighScores||{};topics.forEach(t=>{st.stats[t]||={c:0,n:0};st.battleTopics[t]||={c:0,n:0}});
...
function answerSolo(ch){if(answered)return;answered=true;let ok=ch===current.c;st.stats[current.t]??={c:0,n:0};let s=st.stats[current.t];s.n++;[...$("soloA").children].forEach((b,j)=>{b.disabled=true;let orig=b.dataset.orig!==undefined?Number(b.dataset.orig):j;if(orig===current.c)b.classList.add("correct");if(orig===ch&&!ok)b.classList.add("wrong")});if(mode==="adaptive"){adaptivePerf[current.t]??={c:0,n:0};adaptivePerf[current.t].n++;if(ok)adaptivePerf[current.t].c++;else adaptiveMisses.push({t:current.t,q:current.q})}if(ok){s.c++;correct++;st.streak++;let gain=50+current.d*10;if(mode==="boss")gain*=2;st.xp+=gain;st.seasonXp+=gain;$("soloFeedback").textContent="Correct. Nice work.";if(st.streak>=3)badge("🔥 Chee Hoo!")}else{st.streak=0;badge("💩 Das Not Good");$("soloFeedback").textContent="Das Not Good 💩 Correct answer: "+current.a[current.c]};save();profile();$("soloNext").disabled=false}
