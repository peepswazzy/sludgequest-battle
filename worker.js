...
let st=JSON.parse(localStorage.getItem("sludgequest")||"null")||{xp:0,streak:0,badges:[],stats:{}};st.battleWins=st.battleWins||0;st.battleLosses=st.battleLosses||0;st.battleTopics=st.battleTopics||{};st.seasonXp=st.seasonXp||0;st.seasonWins=st.seasonWins||0;st.seasonLosses=st.seasonLosses||0;st.seasonBossWins=st.seasonBossWins||0;st.seasonId=st.seasonId||"S1";st.operatorId=st.operatorId||("OP-"+Math.random().toString(36).slice(2,10)+Date.now().toString(36).slice(-4));st.operatorName=st.operatorName||"Operator";st.operatorPlant=st.operatorPlant||"Unassigned Plant";st.operatorGrade=st.operatorGrade||4;st.questProgress=st.questProgress||{};st.bestBattleScore=Number(st.bestBattleScore)||0;st.bestBattleType=st.bestBattleType||"";st.battleHighScores=st.battleHighScores||{};topics.forEach(t=>{st.stats[t]||={c:0,n:0};st.battleTopics[t]||={c:0,n:0}});
