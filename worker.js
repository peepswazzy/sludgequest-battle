...
<div class="quest-head"><div><p class="eyebrow">QUEST MODE 3.0</p><h2>Choose Your Process</h2></div><button id="questHome" class="secondary">Home</button></div>
...
{key:"activated",name:"Activated Sludge Quest",icon:"🦠",badge:"🦠 Bug Farmer",questions:[
{t:"Activated Sludge",d:1,q:"In activated sludge, RAS is returned primarily to:",a:["Maintain biological solids in the treatment process","Increase grit removal","Disinfect influent","Reduce primary sludge pumping"],c:0},
{t:"Activated Sludge",d:2,q:"If wasting is reduced while other conditions stay similar, SRT will generally:",a:["Increase","Decrease","Become zero","Equal the DO concentration"],c:0},
{t:"Nitrification",d:3,q:"Aeration DO is acceptable, but effluent ammonia rises after wasting was increased significantly. What should be evaluated?",a:["SRT and nitrifier inventory along with loading and temperature","Screen differential only","Disinfection contact time only","Primary sludge pumping only"],c:0},
{t:"Activated Sludge",d:4,q:"MLSS is stable, but settleability worsens and SVI rises over several days. What is the best next step?",a:["Review sludge age, filament indicators, loading, DO trends, and clarifier performance","Stop RAS immediately","Double disinfectant dose","Increase grit-removal air"],c:0},
{t:"Nitrification",d:5,q:"👹 PROCESS BOSS: A step-feed system shows rising final-pass ammonia while upstream DO appears adequate. What should be verified FIRST?",a:["Actual air distribution, loading split, alkalinity/pH, and available nitrifier mass","Only influent temperature","Only secondary effluent TSS","Digester volatile acids"],c:0}]},
...
function renderQuestHub(){let done=questDoneCount();$("questUnlock").textContent=drcUnlocked()?"👑 Grade 4 / DRC Quest UNLOCKED — go run da boss shift.":"🔒 Complete 6 process trains to unlock Grade 4 / DRC Quest. Progress: "+done+"/6";$("questList").innerHTML="";questTrains.forEach(q=>{let locked=q.key==="drc"&&!drcUnlocked(),p=st.questProgress[q.key]||{},best=Number(p.best||0),pct=Math.round(best/q.questions.length*100);let b=document.createElement("button");b.className="quest-card"+(locked?" locked":"");b.disabled=locked;b.innerHTML="<b>"+q.icon+" "+q.name+(p.completed?" ✅":"")+"</b><small>Mastery "+pct+"% • Best "+best+"/"+q.questions.length+(locked?" • LOCKED":"")+"</small><div class='mastery'><div style='width:"+pct+"%'></div></div>";b.onclick=()=>startQuest(q.key);$("questList").appendChild(b)})}
...
function startQuest(key){activeQuest=questTrains.find(q=>q.key===key);if(!activeQuest)return;if(key==="drc"&&!drcUnlocked()){renderQuestHub();return}mode="quest";i=0;correct=0;deck=activeQuest.questions.map((q,idx)=>({...q,stage:activeQuest.name,stageIcon:activeQuest.icon,level:questLevels[idx]}));soloTotal=deck.length;show("solo");renderSolo()}
