...
function startSolo(m){mode=m;i=0;correct=0;soloTotal=m==="boss"?5:10;let p=[...bank];if(m==="exam")p=p.filter(q=>q.y==="exam");if(m==="alarm")p=p.filter(q=>q.y==="scenario");if(m==="boss")p=p.filter(q=>q.y==="scenario"&&q.d>=4);while(p.length<soloTotal)p=[...p,...p];deck=shuffle(p).slice(0,soloTotal);show("solo");renderSolo()}
function renderSolo(){answered=false;$("soloNext").disabled=true;current=deck[i];let surprise=mode!=="exam"&&mode!=="alarm"&&Math.random()<.15;if(surprise)current=shuffle(bank.filter(q=>q.y==="scenario"))[0];$("soloAlarm").classList.toggle("hidden",!surprise);$("soloMode").textContent=surprise?"BRAH… ALARM!":mode.toUpperCase();$("soloCount").textContent=\`\${i+1}/\${soloTotal}\`;$("soloTopic").textContent=current.t;$("soloQ").textContent=current.q;$("soloFeedback").textContent="Choose the best answer.";$("soloA").innerHTML="";current.a.forEach((a,j)=>{let b=document.createElement("button");b.className="answer";b.textContent=\`\${String.fromCharCode(65+j)}. \${a}\`;b.onclick=()=>answerSolo(j);$("soloA").appendChild(b)})}
...
$("soloNext").onclick=()=>{if(i===soloTotal-1){$("soloQ").textContent=\`Session complete — \${correct}/\${soloTotal}\`;$("soloA").innerHTML="";if(mode==="boss"&&correct>=4){st.seasonBossWins++;badge("👑 DRC LIFE");$("soloFeedback").textContent="👑 DRC BOSS CLEARED — Boss Win +1. Everybody get your number.";save();profile()}else if(mode==="boss"){$("soloFeedback").textContent="Boss shift complete. Score 4/5 or better to earn a Boss Win."}else{$("soloFeedback").textContent="Progress saved on this device."}$("soloNext").disabled=true;syncProfile()}else{i++;renderSolo()}};
