...
<div class="top"><div><p class="eyebrow">SLUDGEMATCH V11.11.1</p><h2>📣 Update Board</h2></div><button id="updateHome" class="secondary">Home</button></div>
...
function show(id){["home","updateBoard","adaptiveSetup","questHub","solo","lobby","waiting","battleGame","results","leaderboard"].forEach(x=>$(x).classList.toggle("hidden",x!==id));if(id==="home")refreshUpdateBadge()}
...
function startSolo(m){mode=m;activeQuest=null;i=0;correct=0;soloTotal=m==="boss"?5:10;if(m==="adaptive"){adaptivePerf={};adaptiveMisses=[];adaptiveUsed=new Set();deck=[pickAdaptive()];show("solo");renderSolo();return}let p=[...bank];if(m==="exam")p=p.filter(q=>q.y==="exam");if(m==="alarm")p=p.filter(q=>q.y==="scenario");if(m==="boss")p=p.filter(q=>q.y==="scenario"&&q.d>=4);while(p.length<soloTotal)p=[...p,...p];deck=shuffle(p).slice(0,soloTotal);show("solo");renderSolo()}
...
$("soloHome").onclick=()=>{if(mode==="quest"){renderQuestHub();show("questHub")}else if(mode==="adaptive"){renderAdaptiveSetup();show("adaptiveSetup")}else show("home")};
